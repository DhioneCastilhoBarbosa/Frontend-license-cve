...
export default function LicenseCharts({ licenses }: LicenseChartsProps) {
  const lineData = getLast12MonthsChartData(licenses);
  const { active, expired, revoked, total } = countByStatus(licenses);

  const pieData = [
    { name: "Ativas", value: active },
    { name: "Expiradas", value: expired },
    { name: "Não Ativadas", value: revoked },
  ].filter((d) => d.value > 0);
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
      <div className="lg:col-span-2 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-5 shadow-sm">
        <h3 className="text-sm font-semibold text-zinc-900 dark:text-white mb-4">
          Licenças Criadas nos Últimos 12 Meses
        </h3>
